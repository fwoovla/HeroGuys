...
var bullet_scene = preload("res://Scenes/Troops/ShotgunBullet.tscn")
...
func init_weapon(_data, _name, _level, _team_num):
	...
	for b in range(max_ammo):
		...
		bul.get_node("RayCast").set_collision_mask_bit(_team_num+1, false)
		bul.name = parent_name + "_sg" + str(b)
...
sync func shoot(pos):
	for position in $Spread.get_children():
		var b = bullet_pool.pop_back()
		#print("boom! #", b, "  ", level_node)
		level_node.add_child(b)
		b.global_transform = position.global_transform
		b.fire(pos)
